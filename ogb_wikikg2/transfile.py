...
class AttentionLayer(nn.Module):
    ...
    def forward(self, x, attention_mask=None):
        ...
        qkv = qkv.reshape(B, L, 3, self.num_heads,
                          self.head_dim)  # [B, L, 3, num_heads, head_dim]
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.attn = AttentionLayer(num_feat=in_feat,
                                   num_heads=num_heads,
                                   qkv_bias=qkv_bias,
                                   dropout_p=dropout_p)

        self.mlp = MLP(in_feat=in_feat,
                       mlp_ratio=mlp_ratio,
                       out_feat=out_feat,
                       dropout_p=dropout_p,
                       act_layer=act_layer)
    ...
    def forward(self, x, attention_mask=None):
        x = x + self.droppath(self.attn(self.norm1(x), attention_mask))
...
class AttentionPoolLayer(nn.Module):
    ...
    def forward(self, x, q):
        ...
        # attn: [B, 1, L]
        # v: [B, L, num_fea]
        x = attn @ v  # [B, L, num_heads, head_dim]
        #x = x.reshape(B, L, self.num_feat)  # [B, L, num_feat]
