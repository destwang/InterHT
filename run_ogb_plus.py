...
from ogb_wikikg2.model_plus import KGEModel
...
def parse_args(args=None):
    ...
    parser.add_argument('--test_log_steps',
                        default=1,
                        type=int,
                        help='valid/test log every xx steps')

    parser.add_argument('--nentity',
                        type=int,
                        default=0,
                        help='DO NOT MANUALLY SET')
    parser.add_argument('--nrelation',
                        type=int,
                        default=0,
                        help='DO NOT MANUALLY SET')

    parser.add_argument('--print_on_screen',
                        action='store_true',
                        help='log on screen or not')
    parser.add_argument(
        '--ntriples_eval_train',
        type=int,
        default=200000,
    ...
    parser.add_argument('--inverses',
                        action='store_true',
                        help='whether to add inverse edges')
    parser.add_argument(
        '--val_inverses',
        action='store_true',
        help='whether to add inverse edges to the validation set')
    parser.add_argument('--tkn_dir',
                        type=str,
                        default="all",
                        help='neighbors direction for igraph')
    parser.add_argument('--drop',
                        type=float,
                        default=0.1,
                        help='Dropout in layers')
...
def save_model(model, optimizer, save_variable_list, args):
    ...
    np.save(os.path.join(args.save_path, 'anchor_embedding'), entity_embedding)

    relation_embedding = model.relation_embedding.weight.detach().cpu().numpy()
    np.save(os.path.join(args.save_path, 'relation_embedding'),
            relation_embedding)
...
def set_logger(args):
    ...
    if args.do_train:
        log_file = os.path.join(args.save_path or args.init_checkpoint,
                                'train.log')
    else:
        log_file = os.path.join(args.save_path or args.init_checkpoint,
                                'test.log')
...
def test_step(model,
    ...
    test_dataloader_head = DataLoader(TestDataset(test_triples, args,
                                                  'head-batch',
                                                  random_sampling),
    ...
    test_dataloader_tail = DataLoader(TestDataset(test_triples, args,
                                                  'tail-batch',
                                                  random_sampling),
                                      batch_size=args.test_batch_size,
                                      num_workers=max(1, args.cpu_num // 2),
    ...
    test_dataloader_head, test_dataloader_tail = accelerator.prepare(
    ...
    with torch.no_grad():
        for test_dataset in test_dataset_list:
            samples_seen = 0
            for step, (positive_sample, negative_sample,
                       mode) in enumerate(test_dataset):
                if args.cuda:
                ...
                if accelerator.num_processes > 1:
                    if step == len(test_dataset) - 1:
                        score = score[:len(test_dataset) - samples_seen]
                    else:
                        samples_seen += score.shape[0]

                batch_results = evaluator.eval({
                    'y_pred_pos': score[:, 0],
                    'y_pred_neg': score[:, 1:]
                })
...
def main(args):
    ...
    if args.inverses:
        ...
        train_triples['relation'] = np.concatenate(
            [train_triples['relation'], train_triples['relation'] + nrelation])
        ...
        if args.val_inverses:
            ...
            orig_head_negs, orig_tail_negs = valid_triples[
                'head_neg'], valid_triples['tail_neg']
            valid_triples['head'] = np.concatenate([orig_head, orig_tail])
            valid_triples['tail'] = np.concatenate([orig_tail, orig_head])
            valid_triples['relation'] = np.concatenate([
                valid_triples['relation'],
                valid_triples['relation'] + nrelation
            ])
            valid_triples['head_neg'] = np.concatenate(
                [orig_head_negs, orig_tail_negs], axis=0)
            valid_triples['tail_neg'] = np.concatenate(
                [orig_tail_negs, orig_head_negs], axis=0)
            ...
            orig_head_negs, orig_tail_negs = test_triples[
                'head_neg'], test_triples['tail_neg']
            test_triples['head'] = np.concatenate([orig_head, orig_tail])
            test_triples['tail'] = np.concatenate([orig_tail, orig_head])
            test_triples['relation'] = np.concatenate([
                test_triples['relation'], test_triples['relation'] + nrelation
            ])
            test_triples['head_neg'] = np.concatenate(
                [orig_head_negs, orig_tail_negs], axis=0)
            test_triples['tail_neg'] = np.concatenate(
                [orig_tail_negs, orig_head_negs], axis=0)
    ...
    logging.info(
        f"Total number of params: {sum(p.numel() for p in kge_model.parameters())}"
    )
    ...
    train_dataloader_head, train_dataloader_tail, model, optimizer = accelerator.prepare(
        train_dataloader_head, train_dataloader_tail, kge_model, optimizer)
    ...
    if args.do_train:
        ...
        if args.do_valid and best_val_metrics != None:
            log_metrics('Best Val  Metrics', best_metrics_step,
                        best_val_metrics, writer)
        if args.do_test and best_test_metrics != None:
            log_metrics('Best Test Metrics', best_metrics_step,
                        best_test_metrics, writer)
